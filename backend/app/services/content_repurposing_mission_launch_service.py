"""Launch one durable content-repurposing run through the Mission core."""
...
from app.content_intelligence.content_mission_contracts import (
    CONTENT_GENERATION_CAPABILITY,
    CONTENT_REPURPOSING_MISSION_NAME,
    CONTENT_REPURPOSING_WORKFLOW,
    ContentRepurposingWorkflowPayload,
    content_repurposing_mission_idempotency_key,
)
...
class ContentRepurposingMissionLaunchResult:
...
class ContentRepurposingMissionLaunchService:
    mission_name = CONTENT_REPURPOSING_MISSION_NAME
    workflow_name = CONTENT_REPURPOSING_WORKFLOW
    required_capability = CONTENT_GENERATION_CAPABILITY
    objective = "repurpose and evaluate approved content"
    ...
    def _result_from_record(cls, record, run_id):
        success = True if record.status == "COMPLETED" else False if record.status == "FAILED" else None
        return ContentRepurposingMissionLaunchResult(
            content_repurposing_run_id=run_id,
            mission_id=record.id,
            mission_status=record.status,
            workflow=record.workflow_name,
            required_capability=record.required_capability,
            idempotency_key=record.idempotency_key or content_repurposing_mission_idempotency_key(run_id),
    ...
    def _fresh_run(self, run_id):
        db = self.session_factory()
        try:
            run = ContentRepurposingRunRepository(db).get_by_id(run_id)
            if run is None:
                raise ValueError("content repurposing run does not exist")
            if run.status != "CREATED":
                raise RuntimeError(f"content repurposing run is already {run.status.lower()}")
            return run.id
        finally:
            db.close()

    def launch(self, content_repurposing_run_id):
        key = content_repurposing_mission_idempotency_key(content_repurposing_run_id)
        existing = self._existing(content_repurposing_run_id, key)
        if existing is not None:
            return existing
        run_id = self._fresh_run(content_repurposing_run_id)
        self.mission_manager.launch(
            name=self.mission_name,
            objective=self.objective,
            workflow=self.workflow_name,
            metadata=ContentRepurposingWorkflowPayload(run_id).to_dict(),
            required_capability=self.required_capability,
            idempotency_key=key,
        )
        result = self._existing(run_id, key)
        if result is None:
            raise RuntimeError("durable content repurposing mission could not be recovered after launch")
        return result
