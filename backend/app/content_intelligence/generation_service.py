...
class ContentGenerationService:
    def __init__(self, db: Session, provider_factory=ContentGenerationProviderFactory, prompt_builder=None, validator=None):
        self.db=db; self.briefs=ContentBriefService(db); self.artifacts=GeneratedContentArtifactRepository(db); self.provider_factory=provider_factory; self.prompt_builder=prompt_builder or GroundedContentPromptBuilder(); self.validator=validator or GroundedGenerationValidator()
    def generate(self, request):
        ...
        if run.status != "CREATED": return ContentGenerationResult(run.id, None, run.status)
        run.transition_to("RUNNING"); self.db.commit()
        ...
        try:
            ...
            if not result.success: return self._fail(run, result.failure or ProviderFailure(ProviderFailureCategory.UNKNOWN_PROVIDER_ERROR, "content provider failed"))
    ...
    def _fail(self, run, failure):
        run.error_summary=failure.safe_message; run.transition_to("FAILED"); self.db.commit()
