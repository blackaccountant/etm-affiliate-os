...
class RetryLifecycleCoordinator:
    ...
    def _ownership_error(self, execution, message):
        """Fail closed when a claimed retry cannot prove its authority."""
        self._event("Retry ownership recovery failure", execution_id=execution.id, error=message)
        return None

    def _restore_recoverable(self, execution, authority, error):
        """Undo preparation failure before lease acquisition without spending retry budget."""
        if execution is not None and authority is not None:
            return self.execution_service.restore_due_retry_claim(
                execution.id, authority, error,
            )
        return False
    ...
    def execute(self, task):
        ...
        if authority is None:
            return self._ownership_error(execution, "Claimed retry lacks lease authority")
        if (
        ...
        try:
            execution = self.execution_service.verify_active_authority(authority)
        except Exception:
            return self._ownership_error(execution, "Retry lease authority is no longer active")
        ...
        expected_mission_status = MissionStatus.RUNNING.value
        ...
        try:
            ...
            self._restore_recoverable(execution, authority, str(exc))
