        ...
        try:
            ...
            if state.current_state == "CREATED":
                ...
                result = db.execute(update(ColdDeliveryOperationState).where(
                    ...
                    ColdDeliveryOperationState.revision == state.revision,
                    execution_is_owned,
                ...
                ).values(current_state="READY", revision=state.revision + 1,
                ...
                db.add(ColdDeliveryEvent(operation_id=values.cold_delivery_operation_id,
                    sequence_number=state.next_event_sequence, event_type="RUNTIME_READY", occurred_at=now,
                    source_namespace="cold-b2b-runtime-v1",
                    source_event_key=f"{values.cold_delivery_operation_id}:{authority.execution_id}:{authority.lease_generation}:{state.revision}",
                    event_fingerprint=sha256_fingerprint({"operation_id": values.cold_delivery_operation_id, "state": "READY", "execution_id": authority.execution_id, "generation": authority.lease_generation, "revision": state.revision}),
