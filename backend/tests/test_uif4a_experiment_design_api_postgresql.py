...
DATABASE = "etm_uif4a_experiment_design_api_qualification"
ROLE = os.getenv("ETM_UIF4A_DB_ROLE")
RAW = os.getenv("ETM_UIF4A_DATABASE_URL")
FRESH = os.getenv("ETM_UIF4A_DB_FRESHNESS_ATTESTED")

if not RAW:
    pytest.skip("requires guarded UIF4A URL", allow_module_level=True)
...
if (
    ...
    raise RuntimeError("UIF4A database guard failed")
...
def _design(program_id, reference):
    return {
        "experiment_reference": reference,
        "approved_dimensions": _dimension(program_id),
        "hypothesis": f"hypothesis-{reference}",
        "control_definition": "existing-strategy",
        "treatment_definition": f"treatment-{reference}",
        "success_measure": "operating-profit-evidence",
        "observation_window": "P14D",
        "design_reference": f"design-{reference}",
        "designed_at": "2100-01-02T00:00:00Z",
    }
...
def _payload(
    ...
    return {
        ...
        "experiment_design_policy": {
            "policy_version": "qualification-experiment-design-v1",
        },
    }
...
def _ids(rows):
    return [
        item["approved_recommendation_row"]["dimensions"][0]["value"]
        for item in rows
    ]


def _assert_no_http_writes(statements):
    forbidden = (
        "INSERT ",
        "UPDATE ",
        "DELETE ",
...
def test_current_head_requires_no_uif4a_migration():
    db = _session()
    try:
        assert (
...
def test_real_http_approved_design_binds_exact_tier_one_member_without_writes():
    ...
    response, statements = _post(
        _payload(
            ...
            "uif4a-approve-x",
            [_design(x["program"], "experiment-x")],
        )
    )
    ...
    assert _ids(body["experiment_designs"]) == [x["program"]]
    assert z["program"] not in _ids(body["experiment_designs"])
    assert row["experiment_reference"] == "experiment-x"
    assert row["actor_reference"] == "pg-qualification"
    assert row["decision_reference"] == "uif4a-approve-x"
    assert row["approval_policy_version"] == "qualification-approval-v1"
    assert row["experiment_design_policy_version"] == (
        "qualification-experiment-design-v1"
    )
    assert row["source_approval_semantics"] == (
        ECONOMIC_RECOMMENDATION_APPROVAL_SEMANTICS
    )
    ...
    assert row["approved_recommendation_row"]["preference_tier"] == 1
    Decimal(row["approved_recommendation_row"]["operating_profit"])
    _assert_no_http_writes(statements)


def test_real_http_multiple_design_inputs_normalize_to_frozen_approval_order():
    x, _y, z = _seed((2, 1, 2), "CAD")

    response, statements = _post(
        _payload(
            "CAD",
            "APPROVED",
            [_dimension(x["program"]), _dimension(z["program"])],
            "uif4a-tie",
            [
                _design(z["program"], "experiment-z"),
                _design(x["program"], "experiment-x"),
            ],
        )
    )
    ...
    assert response.status_code == 200
    rows = response.json()["experiment_designs"]
    assert _ids(rows) == [x["program"], z["program"]]
    assert [row["experiment_reference"] for row in rows] == [
        "experiment-x",
        "experiment-z",
    ]
    _assert_no_http_writes(statements)
...
@pytest.mark.parametrize(
    ("state", "currency"),
    [("REJECTED", "GBP"), ("DEFERRED", "USD")],
)
def test_real_http_nonapproval_states_return_no_designs_without_writes(
    state,
    currency,
):
...
def test_uif4a_route_source_adds_no_network_or_execution_client():
